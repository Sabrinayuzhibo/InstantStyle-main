...
loaded_target = "Load only style blocks"
...
def create_image(image_pil,
                 ...
                 neg_content_scale=0):
    global ip_model, loaded_target

    # Rebuild adapter only when style mode changes to avoid repeated GPU memory growth.
    if target != loaded_target:
        del ip_model
        gc.collect()
