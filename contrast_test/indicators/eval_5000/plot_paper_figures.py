...
def plot_tradeoff_scatter(results: List[Dict]) -> None:
    ...
    sizes = 160 + (lpips_vals - lpips_vals.min()) / (lpips_vals.ptp() + 1e-8) * 520
    ...
    for r, size in zip(results, sizes):
        ...
        marker = "*" if is_ours else "D" if is_reference_ours else "s" if is_baseline else "o"
        ax.scatter(
            r["fid"],
            r["art_fid"],
            s=650 if is_ours else 300 if is_reference_ours else size,
            c=method_color(r["name"]),
            marker=marker,
            edgecolors="black" if is_ours or is_reference_ours or is_baseline else "white",
            linewidths=1.2 if is_ours or is_reference_ours or is_baseline else 0.8,
            alpha=0.96,
            zorder=5 if is_ours or is_reference_ours or is_baseline else 3,
        )
        label = display_name(r["name"]).replace("\n", " ")
        if is_ours or is_reference_ours:
            ax.annotate(
                label,
                (r["fid"], r["art_fid"]),
                xytext=(16, -2 if is_ours else 12),
            ...
            text_offsets = {
                "adain_only_no_controlnet_1blk": (12, 4, "left", "center"),
                "adain_1blk": (10, 2, "left", "center"),
                "noadain_1blk": (12, 3, "left", "center"),
                "adain_2blk": (12, 3, "left", "center"),
    ...
    ax.set_xlabel("Content distribution distance: FID ↓")
    ax.set_ylabel("Style distribution distance: Art-FID ↓")
    ax.set_title("Content–Style Trade-off on 5k Evaluation Set")
...
def plot_metric_small_multiples(results: List[Dict]) -> None:
    ...
    fig.suptitle("Quantitative Comparison Across Evaluation Metrics", y=1.04, fontweight="bold")
    fig.tight_layout()
...
def plot_relative_improvement(results: List[Dict]) -> None:
    ...
    note = "Ours = proc05_adainfalse_gamma0.6.\nPositive values indicate lower error/distance.\nBaselines are w/o AdaIN 1-block and w/o AdaIN 2-block."
    ax.text(0.98, 0.04, note, transform=ax.transAxes, ha="right", va="bottom", fontsize=8.5, bbox=dict(boxstyle="round,pad=0.35", fc="#F7F7F7", ec="#BBBBBB", lw=0.7))
    fig.tight_layout()
