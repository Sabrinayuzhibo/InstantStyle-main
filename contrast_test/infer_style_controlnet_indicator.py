...
# Make repo-root packages importable when running from contrast_test directory.
...
def _load_pairs_jsonl(jsonl_path: Path, require_control_path: bool = True) -> List[Dict[str, Any]]:
    ...
    with jsonl_path.open("r", encoding="utf-8") as f:
        for line_no, raw in enumerate(f, start=1):
            ...
            if require_control_path and "control_image_path" not in record and "canny_image_path" not in record:
                raise ValueError(
...
def main() -> None:
    ...
    use_controlnet = bool(paths_cfg.get("use_controlnet", True))
    ...
    append_default_prompt_to_jsonl_prompt = bool(
        generate_cfg.get("append_default_prompt_to_jsonl_prompt", True)
    )
    read_negative_prompt_from_jsonl = bool(generate_cfg.get("read_negative_prompt_from_jsonl", False))
    resume_if_exists = bool(generate_cfg.get("resume_if_exists", True))
    ...
    # In comparison experiments, keep each run isolated under its configured output_dir.
    prefer_output_dir_over_task_output = bool(
        output_cfg.get("prefer_output_dir_over_task_output", True)
    )
    ...
    if pairs_jsonl_path:
        ...
        tasks = _load_pairs_jsonl(pairs_file, require_control_path=use_controlnet)
    ...
    def _expected_save_base(task_idx: int, task: Dict[str, Any], style_file: Path, control_file: Optional[Path], canny_file: Optional[Path]) -> Path:
        custom_output = task.get("output_image_path")
        use_task_output_path = bool(custom_output) and not (
            output_dir_cfg and prefer_output_dir_over_task_output
        )
        if use_task_output_path:
            return _resolve_path(str(custom_output), config_base_dir)
        if custom_output and output_dir_cfg and prefer_output_dir_over_task_output:
            custom_name = Path(str(custom_output)).name
        ...
        return output_dir / f"{base_name}.jpg"

    start_task_idx = 1
    if resume_if_exists and tasks:
        for task_idx, task in enumerate(tasks, start=1):
            style_file = _resolve_path(str(task["style_image_path"]), config_base_dir)
            if not style_file.exists():
                break
            control_file = None
            task_control_image_path = task.get("control_image_path")
            if task_control_image_path:
                control_file = _resolve_path(str(task_control_image_path), config_base_dir)
                if not control_file.exists():
                    break
            canny_file = None
            task_canny_image_path = task.get("canny_image_path")
            if task_canny_image_path:
                canny_file = _resolve_path(str(task_canny_image_path), config_base_dir)
                if not canny_file.exists():
                    break

            save_base = _expected_save_base(task_idx, task, style_file, control_file, canny_file)
            save_paths = [save_base]
            if num_samples > 1:
                save_paths = [save_base.with_name(f"{save_base.stem}_s{sample_idx:02d}{save_base.suffix}") for sample_idx in range(1, num_samples + 1)]
            if not all(p.exists() for p in save_paths):
                break
            start_task_idx = task_idx + 1
        if start_task_idx > 1:
            print(f"[info] resume mode: skipping already completed tasks 1..{start_task_idx - 1}")

    for task_idx, task in enumerate(tasks[start_task_idx - 1 :], start=start_task_idx):
        ...
        if use_controlnet and control_file is None and canny_file is None:
            raise ValueError(
                f"Task [{task_idx}] must provide control_image_path or canny_image_path when use_controlnet is true"
            )
        if not use_controlnet and control_file is None and canny_file is None:
            print(f"[info] task {task_idx}/{len(tasks)} use_controlnet=false, ignoring control_image_path/canny_image_path if present")
        elif not use_controlnet and control_file is not None:
            print(f"[info] task {task_idx}/{len(tasks)} use_controlnet=false, ignoring control image={control_file}")
        ...
        if use_controlnet:
            ...
            if control_file is not None:
                print(f"[info] task {task_idx}/{len(tasks)} use_controlnet=false, ignoring control image={control_file}")
            control_w, control_h = style_image.size
        ...
        if read_prompt_from_jsonl:
            jsonl_prompt = str(task.get("prompt", "")).strip()
            default_prompt = prompt.strip()
            if jsonl_prompt:
                if append_default_prompt_to_jsonl_prompt and default_prompt:
                    task_prompt = f"{jsonl_prompt}, {default_prompt}"
                else:
                    task_prompt = jsonl_prompt
        ...
        if use_task_output_path:
            ...
            save_paths = [
                save_base if sample_idx == 1 else save_base.with_name(f"{save_base.stem}_s{sample_idx:02d}{save_base.suffix}")
                for sample_idx in range(1, num_samples + 1)
            ]
        else:
            # If task output path exists, preserve its filename but place it under output_dir.
            if custom_output and output_dir_cfg and prefer_output_dir_over_task_output:
                custom_name = Path(str(custom_output)).name
                save_base = output_dir / custom_name
                save_paths = [
                    save_base if sample_idx == 1 else save_base.with_name(f"{save_base.stem}_s{sample_idx:02d}{save_base.suffix}")
                    for sample_idx in range(1, num_samples + 1)
                ]
                ...
                base_name = f"{task_idx:04d}_sty_{style_file.stem}_cnt_{content_stem}"
                save_paths = [
                    output_dir / f"{base_name}.jpg" if sample_idx == 1 else output_dir / f"{base_name}_s{sample_idx:02d}.jpg"
                    for sample_idx in range(1, num_samples + 1)
                ]

        if resume_if_exists and all(p.exists() for p in save_paths):
            print(f"[info] task {task_idx}/{len(tasks)} already exists, skip: {save_paths[0]}")
            continue
